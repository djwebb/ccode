...
!-----------------------------------------------------------------------
!
   integer (int_kind) :: i_djw,j_djw,icount_djw = 0
   real (r8)          :: depth1_djw, depth2_djw, dtime_djw,  &
                         rlon_djw,  rlat_djw,                &
                         sst_djw1,  sst_djw2, sst_djw,       &
                         coef_djw
!
!-----------------------------------------------------------------------
!
...
!
#if 1
   if(icount_djw.lt.100.and.k.eq.1)then
     icount_djw = icount_djw + 1
     write(stdout,'(a,i5))')   &
        " ### DJW :: Routine tracer_update.  icount_djw = ", icount_djw
#if 1
     do i_djw=1,nx_block,10
     do j_djw=1,ny_block,10
         write(stdout,'(a,5i5,i6,7f10.4))')                          &
         " ## AA ",i_djw,j_djw,k,nx_block,ny_block,bid,              &
         tlon(i_djw,j_djw,bid)*radian, tlat(i_djw,j_djw,bid)*radian, &
         tlon(i_djw,j_djw,bid), tlat(i_djw,j_djw,bid),               &
         told(i_djw,j_djw,k,1)
     enddo
     enddo
#endif
   endif
!
!  Timestep of ocean model is 10800s (3 hours)
!  I decided relaxation was probably too strong - increased scale time to 2 days
!
   depth1_djw =  3000.0_r8
   depth2_djw = 10000.0_r8
   dtime_djw  = 86400.0_r8*2.0_r8
   sst_djw1   =    29.5_r8
   sst_djw2   =    28.5_r8


   if(zt(k).le.depth2_djw)then
     coef_djw = 1.0_r8/dtime_djw
     if(zt(k).gt.depth1_djw)then
        coef_djw = coef_djw*(zt(k)-depth2_djw)/(depth1_djw-depth2_djw)
     endif

     do i_djw=1,nx_block
     do j_djw=1,ny_block
     if(k <= KMT(i_djw,j_djw,bid).and.                  &
            tlon(i_djw,j_djw,bid)*radian >= 180.00_r8 .and.  &
            tlon(i_djw,j_djw,bid)*radian <= 240.00_r8 .and.  &
            tlat(i_djw,j_djw,bid)*radian >=   4.00_r8 .and.  &
            tlat(i_djw,j_djw,bid)*radian <=   7.25_r8)then

        sst_djw = sst_djw1  + (sst_djw2-sst_djw1)            &
                              *(tlon(i_djw,j_djw,bid)*radian-180d0)/60d0
        FT(i_djw,j_djw,1) = FT(i_djw,j_djw,1)                &
                            + coef_djw*(sst_djw - told(i_djw,j_djw,k,1) )
       endif
     enddo
     enddo
   endif
